...
const init: any = function(): void {
    $( '.js-collapsible-text' ).each( function(): Object {
        const maxHeight: number = parseInt( $( this ).data( 'height' ), 10 ) || 100;
        const $content: Object = $( this ).find( $( this ).data( 'content' ) );
        const $trigger: Object = $( this ).find( $( this ).data( 'trigger' ) );
        ...
        if ( $content.length && $content.height() > maxHeight ) {
            ...
            $trigger.on( 'click', function( event: Event ): void {
                ...
                if ( isContentHidden ) {
                    ...
                    $content.css( 'height', 'auto' ).removeClass( `${namespace}collapsible-text__content--hidden` );
                    isContentHidden = false;
                } else {
                    $( this ).text( showMoreText );
                    $content.css( 'height', maxHeight ).addClass( `${namespace}collapsible-text__content--hidden` );
                    isContentHidden = true;
                }
            } );
