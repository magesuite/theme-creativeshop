...
export default class AddressAutofill {
    ...
    private _initGoogleStreetRequest(): void {
        ...
        if ( this._detectHouseNr( query ) ) {
            ...
            this.googlePlaceDetector.getResults( query ).then(
                ( data: any ) => {
                    if ( data ) {
                        this._buildAutosuggestSelect( data );
                    }
    ...
    private _buildAutosuggestSelect( data: any ): void {
        ...
        if ( !$autosuggestSelect.length ) {
            ...
            $autosuggestSelect.find( '.cs-html-select__menu-list' ).empty().append( optionsHtml );
        }
        ...
        setTimeout(
            () => {
                ...
                $jsSelect.find( '.cs-html-select__menu-item' ).eq( 0 ).addClass( 'cs-html-select__menu-item--focused' );
                this._initSelectEvents( $jsSelect );

                // If there is not house number add reminder note
                if ( this.detectorType === 'place' && !$( '.cs-input__note--reminder' ).length ) {
                    $jsSelect.after('<div class="cs-input__note cs-input__note--reminder">' + $translate('Do not forget about <strong>') + $translate('street number') + '</strong>.</div>');
                } else if ( this.detectorType === 'address' ) {
                    this.streetField.parents( '.cs-input' ).find( '.cs-input__note' ).remove();
                }
    ...
    private _buildOptions( data: any ): string {
        ...
        for ( const result of data ) {
            ...
            if ( this.detectorType === 'place' ) {
                ...
                streetNumber = this._detectHouseNr( result.structured_formatting.main_text );

            } else if (this.detectorType === 'address' ) {
                address = this.googleAddressDetector.getFormattedAddress( result);
            }

            const addressZip: string = address.postalCode ? address.postalCode : '';
            const addressCity: string = address.city;
            const addressStreet: string = address.street;
            const addressCountryCode: string = this.detectorType === 'address' ? this._getCountryCodeFromResult( result ) : this.options.region;
            ...
            if ( addressStreet && addressCity ) {
                optionsHtml = optionsHtml + `<li class="cs-html-select__menu-item" data-value='${dataValues}'><a class="cs-html-select__menu-link">${address.full}</a></li>`;
            }
        }
    ...
    private _initSelectEvents( jsSelect: JQuery ): void {
        ...
        function scrollToOption( $items: JQuery, selectedIndex: number, $menu: JQuery ): void {
            // scroll to selected option
            let offset: number = $items.eq( selectedIndex )[ 0 ].offsetTop - $menu[ 0 ].offsetTop;
            offset = offset - $menu[ 0 ].offsetHeight / 2 + $items.eq( 0 ).height() / 2;
            $menu.animate(
                {
                    scrollTop: offset,
                },
        ...
        this.streetField.on( 'keyup', ( e: KeyboardEvent) => {
            ...
            if ( e.which === 38 ) {
                if ( selectedIndex > 0 ) {
                    ...
                    $items.eq( selectedIndex ).addClass( 'cs-html-select__menu-item--focused' );
    ...
    private _fillFields( clickedValues: object ): void {
        ...
        if ( this.countrySelect && ( this.countrySelect.val() !== clickedValues.countryCode ) ) {
