...
export class Flyout {
    ...
    protected settings: FlyoutSettings;
    protected contentOutClass: string;
    protected contentInClass: string;
    protected contentTransitioningClass: string;
    protected triggerInClass: string;
    protected isTransitioning: boolean = false;
    protected $element: JQuery;
    protected $triggers: JQuery;
    protected eventListeners: IFlyoutEventListeners = {};
     ...
     * @param  {FlyoutSettings} settings Optional component settings.
     */
    public constructor( $element: JQuery, settings: FlyoutSettings ) {
     ...
     */
    public show(): void {
        if ( this.isTransitioning || this.$content.hasClass( this.contentInClass ) || this.shown === true ) {
            return;
        }
    ...
    public showComplete(): void {
        ...
        if ( this.settings.type === 'collapse' ) {
            let scrollHeight: number = this.$content[0].scrollHeight;
            this.$content.css( { 'min-height': scrollHeight + 'px' } );
     ...
     */
    public hide(): void {
        if (this.isTransitioning || this.$content.hasClass( this.contentTransitioningClass ) || this.shown === false ) {
            return;
        }
     ...
     */
    public closeOnClickOutside( event: Event): void {

        if ( this.shown === false || this.settings.type === 'collapse' ) {
            return;
        }
...
class FlyoutCollection {
    ...
    public constructor( settings: FlyoutSettings ) {
...
export { FlyoutSettings, FlyoutCollection };
