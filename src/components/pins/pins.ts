...
export default class Pins {
    ...
    protected resolveCardBreakpoint(config?: IPinsConfig): number {
        const name =
            ...
            deepGet(
                viewXml,
                'vars.MageSuite_ContentConstructor.teaser.pins.card_breakpoint'
            );
    ...
    protected setRichPinRoles(): void {
        const pins = this.element.querySelectorAll<HTMLElement>(
            '.cs-pins__pin--rich'
        );
        ...
        if (pin.getAttribute('data-trigger') === 'hover') {
            ...
            if (card) {
                ...
                card.addEventListener('mouseleave', () =>
                    this.scheduleClose(pin)
                );
            }
        }

        trigger.addEventListener('click', (event: Event) =>
            this.activateRichPin(pin, event)
        );
    ...
    protected scheduleClose(pin: HTMLElement): void {
        Pins.cancelClose();
        Pins.closeTimer = window.setTimeout(
            () => Pins.closeCard(pin),
            this.closeDelay
        );
    }
    ...
    protected positionCard(pin: HTMLElement): void {
        ...
        for (const side of order) {
            ...
            const candidateShift: number = this.overflowShift(
                side,
                rect,
                boxRect
            );
