/**
 * Custom validation module replacement that introduces better UX.
 * Parts that have been changed are indicated with // -- comments.
 */
/**
...
(function (factory) {
    ...
    $.widget("mage.validation", {
        options: {
            meta: "validate",
            // ----------------------------------------
            // Adds better validation triggered on field focus out.
            onfocusout: $.validator.defaults.onfocusout,
            // Validates on keyup but only when user corrects invalid field.
            onkeyup: function (element, event) {
                if (event.which == 9 && this.elementValue(element) === '') {
                    return;
                } else if ($(element).hasClass(this.settings.errorClass) && (element.name in this.submitted || element === this.lastActive)) {
