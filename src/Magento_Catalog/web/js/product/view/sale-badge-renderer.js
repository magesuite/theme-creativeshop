...
define([
    ...
    $.widget('magesuite.saleBadgeRenderer', {
        options: {
            ...
            discountBadgeValueSelector: '.cs-page-product__badge-discount-value',
        ...
        _create: function () {
            ...
            if (!this.element.is(this.options.buyBoxSelector)) {
                ...
                this.$tileOrBuybox = this.element.closest(this.options.tileSelector);
            }
        ...
        renderSaleBadge: function () {
            this.$tileOrBuybox.on('change', 'input.swatch-input.super-attribute-select', () => {
        ...
        updateSaleBadge: async function () {
            ...
            var $rootElement = this.options.isInTile ? this.$tileOrBuybox : $(document);
            let swatchRendererElement = $rootElement.find(this.options.productsIdsSelector);
            ...
            var selectedProductDiscounts = _.filter(allDiscountsList, function (key, value) {
                return $productsIdsIndex[value];
            });

            var maximumDiscount = Math.max.apply(null, selectedProductDiscounts);

            var selectedProductId = this.getSelectedProductId($productsIdsIndex);

            var $discountBadge = $rootElement.find(this.options.discountBadgeSelector);
            var $discountBadgeValue = $rootElement.find(this.options.discountBadgeValueSelector);
            var $discountBadgeText = $rootElement.find(this.options.discountBadgeTextSelector);
            ...
            var displayBadge =
                !isProductSelected || (isProductSelected && selectedProductDiscount !== 0);
            ...
            $discountBadgeValue.text(isProductSelected ? selectedProductDiscount : maximumDiscount);
        },

        getSelectedProductId: function (productsIdsIndex) {
            var selectedOptions = {};
            var $attributesList = this.$tileOrBuybox.find(this.options.attributesSelector);
            var productId;
            ...
            return typeof productId === 'undefined' ? 0 : parseInt(productId, 10);
        },
        ...
        getSelectedOptions: function (attributesList) {
            ...
            if ($attributesList.length === 1) {
                ...
                var optionSelected = $attributesList.attr('data-option-selected');

                selectedOptions[attributeId] = optionSelected;
            } else {
                $attributesList.filter('[data-option-selected]').each(function () {
                    var $attribute = $(this);
                    var attributeId = $attribute.attr('data-attribute-id');

                    var optionSelected = $attribute.attr('data-option-selected');
