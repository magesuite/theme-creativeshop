...
export default class Reviews {
    ...
    public constructor( $wrapper?: JQuery, options?: ReviewsOptions ) {
        ...
        this._$feedbackEl       = this._options.feedbackElementSelector ? $( this._options.feedbackElementSelector ) : $( '.cs-reviews__rate-feedback' );
    ...
    protected _attachEvents(): void {
        ...
        for ( let i: number = 0; i < $radios.length; i++ ) {
            $radios[ i ].addEventListener( 'change', function(): void {
                if ( _this._$wrapper.find( 'input[type="radio"]:checked' ).length ) {
                    msg = _this._$wrapper.find( 'input[type="radio"]:checked' ).data( 'feedback-message' );
                } else {
