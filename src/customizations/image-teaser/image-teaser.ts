    ...
    (index: number, element: any): void => {
        ...
        new ImageTeaser($(element), {
            onLazyImageReady(swiper: any, slide: any, image: any): void {
                ...
                if (
                    images.length >= swiper.params.slidesPerView &&
                    swiper.container.hasClass(
                        'cs-image-teaser__wrapper--content-display-outside'
                    ) &&
                    swiper.prevButton.length &&
                    swiper.nextButton.length
                ) {
                    ...
                    const setNavButtonsPosition: any = function(): void {
                        if (
                            ...
                            $(element).hasClass('cs-image-teaser--slider')
                        ) {
                            let h: number = 0;

                            swiper.slides.each(
                                (idx: number, el: any): void => {
                                    const $img: any = $(el).find(
                                        '.cs-image-teaser__image'
                                    );
                                    if ($img.length && $img.outerHeight() > h) {
                                        $tallestImage = $(el).find(
                                            '.cs-image-teaser__image'
                                        );
